...
crit.budworm <- 40 * init.budworm
crit.pct <- 5 * sample(10:17, 1)
@

<<echo=FALSE>>=
winnerModel <- getModel(wks, winner, modelType)
otherModel <- getModel(wks, winner, otherType)
winnerRes <- res(winnerModel, winner)
@
...
\begin{compactenum}
    ...
    \item The pesticide is only effective if it is applied before the
    population growth rate ever exceeds \SI{\Sexpr{crit.pct}}{\%} of its
