...
sqkm.cost <- 10 * sample(8:12, 1)
budworm.cost <- 0.01 * sample(3:6, 1)
forest.budget <- 1e6
#winner
@
...
\begin{compactenum}
    ...
    will persist.
    \item There is a fixed cost of \Sexpr{sqkm.cost} per square kilometer and variable
    cost of \Sexpr{budworm.cost} per thousand budworm larvae per square meter. The budget
    for your forest is \Sexpr{forest.budget}.
\end{compactenum}
...
<<echo=FALSE>>=
  ...
  rate_too_late <- rate_exceeded_time < 2
@
