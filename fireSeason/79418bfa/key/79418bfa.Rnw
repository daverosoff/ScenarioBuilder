...
inline_hook <- function(x) {
    if (is.numeric(x)) {
        # if integer, print without decimal; otherwise, four places
...
maxtemp <- sample(12:19,1) + runif(1) # degrees celsius
precip <- sample(54:66,1) + runif(1)# mm
excess <- sample(22:37,1) + runif(1) # degree-day
density <- sample(6:12, 1) + runif(1)# kg per m^2
season <- sample(10*9:15, 1) + sample(0:9,1) # day
@

<<setup2, echo=FALSE>>=
x <- sample(1:2, 1)
y <- 1/2 + 1/2*sample(0:2, 1)
z <- -1/2*sample(0:2, 1)
w <- z - 1/2*sample(0:2, 1)
@
...
% \vspace{-2.8ex}
    ...
    \begin{compactitem}
        ...
        $[K] = L^{\Sexpr{x}} M^{\Sexpr{y}} \Theta^{\Sexpr{z}} T^{\Sexpr{w}}$.
        ...
        \SI{\Sexpr{maxtemp}}{\celsius}. The January--June average
        accumulated precipitation in 2017 was
        \SI{\Sexpr{precip}}{mm}. The density of vegetation is
        estimated to be about $\SI{\Sexpr{density}}{kg/m^2}$ in this region.
        The May--June average cumulative temperature excess for 2017 was
        $\SI{\Sexpr{excess}}{\celsius.day}$. Finally, the 2017 fire season
        lasted $\num{\Sexpr{season}}$ days. You will base your model on these
        data.
...
<<solution, echo=FALSE>>=
knit_hooks$set(inline=old_inline)
a <- w - 1 - z
b <- -2*y - x
c <- -y
d <- 1 - w
K.no.convert <- season / (maxtemp^a * precip^b * density^c * excess^d)
K.meters <- K.no.convert/(1000^b)
@

Formula: $S = \Sexpr{K.no.convert} Q^{\Sexpr{a}} P^{\Sexpr{b}} \delta^{\Sexpr{c}} X^{\Sexpr{d}}$

Formula if convert to meters: $S = \Sexpr{K.meters} Q^{\Sexpr{a}} P^{\Sexpr{b}} \delta^{\Sexpr{c}} X^{\Sexpr{d}}$
